...
class ConvolutionalNeuralNet:
    ...
    def get_train_test_tranfomers(
        self, w, h, mean_train, std_train, mean_test, std_test
    ):
        ...
        # For albumentations transformation
    ...
    def select_images_data_sets(self):
        ...
        # A colon on the left side of an index means everything before, but not including, the index; All the data up to
        # the beginning of 80% of the train set, which is 80%
        # A colon on the right side of an index means everything after the specified index : All the data after 80%
        # of the data, which is 20%
        # train_image_paths, test_image_paths = train_image_paths[:int(0.5*len(train_image_paths))],
        #   train_image_paths[int(0.5*len(train_image_paths)):]
    ...
    def __create_transformators_train_test(self):
        ...
        train_stats = StatsFromDataSet(
            data_path=self.train_data_path, batch_size=self.batch_size
        )
        train_normalized_loader = train_stats.init_large_loader(
            w=self.img_width, h=self.img_height
        )
        mean_train_set, std_train_set = train_stats.batch_mean_and_std(
            train_normalized_loader
        )

        test_stats = StatsFromDataSet(
            data_path=self.test_data_path, batch_size=self.batch_size
        )
        test_normalized_loader = test_stats.init_large_loader(
            w=self.img_width, h=self.img_height
        )
        mean_test_set, std_test_set = test_stats.batch_mean_and_std(
            test_normalized_loader
        )

        # Get transformers using the mean and the std from original dataset
        train_tranforms, test_transforms = self.get_train_test_tranfomers(
            self.img_width,
            self.img_height,
            mean_train=mean_train_set.tolist(),
            std_train=std_train_set.tolist(),
            mean_test=mean_test_set,
            std_test=std_test_set,
        )
    ...
    def create_datasets(self, train_image_paths, test_image_paths, classes):
        ...
        train_dataset = GenderDataset(
            train_image_paths, classes, transform=train_tranforms
        )
        test_dataset = GenderDataset(
            test_image_paths, classes, transform=test_transforms
        )
        print(
    ...
    # Define train model
    def train_k_fold(
        self, train_dataset, test_dataset, num_epochs: int = 10, num_folds: int = 5
    ):
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(train_dataset), 1):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                train_loss, train_acc = self.train(
                    train_loader, loss_function, optimizer
                )
                print(
            ...
            #     print (f'Model\'s test accuracy ({test_accuracy}) on fold {fold + 1} is higher than best accuracy 
            #     ({best_accuracy}). Saving model..')
            #     best_accuracy = test_accuracy
            #     best_fold = fold
            # else :
            #     print (f'Model\'s current accuracy ({test_accuracy}) on fold{fold + 1} is lower than the best 
            # accuracy {best_accuracy}. Model will not be saved')
    ...
    def evaluate_dataset(
        self, valid_loader: DataLoader, classes, num_images_plot: int = None
    ):
        self.network.eval()
        ...
        with torch.no_grad():
            ...
            for i, (inputs, labels) in enumerate(valid_loader):
                ...
                if i == 0:
                    ...
                    ground_true_labels = [
                        classes[labels[i]] for i in range(len(inputs))
                    ]
        ...
        print(
            f"Accuracy of the network on {len(valid_loader.dataset)} validation images: {selected_images_acc} %"
        )
        ...
        preprocess = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    mean=[0.6527503132820129, 0.48301947116851807, 0.4047924280166626],
    ...
    def plot_prediction(self, prediction, image):
        title = (
            [*prediction.keys()][0].upper()
            + " => Probability: "
            + "{:,.2f} %".format(float([*prediction.values()][0]) * 100)
        )
        # Plot
        plt.imshow(image)
        plt.title(title)
    ...
    def plot_loss_and_accuracy(self, train_loss, train_acc, test_loss, test_acc):
        ...
        plt.title(
            "Mean of 10-Fold Cross-Validation Accuracy and Loss of Train and Validation"
        )
